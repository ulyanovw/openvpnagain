...
perdel(){
			...
			if [[ "$number_of_clients" = 0 ]]; then
				echo
				echo "Клиенты отсутсвуют, кого вы хотите пердел?!"
				exit
			fi
			echo
	...
	new_client
	echo
	echo "$client пердел. Конфигурация доступна в:" ~/"$client.ovpn"
	rm /home/admin/"$client.ovpn"
	cp ~/"$client.ovpn" /home/admin/
	exit
}
showlink(){
	number_of_clients=$(tail -n +2 /etc/openvpn/server/easy-rsa/pki/index.txt | grep -c "^V")
...
echo -e "Приветствую, администратор сервера!  Дата: $(date +"%d-%m-%Y")
  ...
  ${Green_font_prefix}3.${Font_color_suffix} Пердел
